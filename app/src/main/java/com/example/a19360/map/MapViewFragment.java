...
public class MapViewFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        MapStatus mMapStatus = new MapStatus.Builder()//定义地图状态
                .target(latLng).zoom(17).build();//定义MapStatusUpdate对象，以便描述地图状态将要发生的变化

        MapStatusUpdate mMapStatusUpdate = MapStatusUpdateFactory.newMapStatus(mMapStatus);
        myBaiduMap.setMapStatus(mMapStatusUpdate);//改变地图状态

        BitmapDescriptor bitmap = BitmapDescriptorFactory.fromResource(R.drawable.mark);
        //准备 marker option 添加 marker 使用
        MarkerOptions markerOption = new MarkerOptions().icon(bitmap).position(latLng);
        //获取添加的 marker 这样便于后续的操作
        myBaiduMap.addOverlay(markerOption);

        OverlayOptions textOption = new TextOptions().bgColor(0xAAFFFF00).fontSize(50).
                fontColor(0xFFFF00FF).text("百度地图SDK").rotate(0).position(latLng);
